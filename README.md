# FTC_OpenCV - OpenCV 攝影機展示專案 & FTC 比賽分析器

這是一個 Java Swing 應用程式，包含兩個主要功能：
1. **OpenCV 攝影機展示** - 展示如何使用 OpenCV 從攝影機擷取即時影像
2. **FTC 比賽分析器** - FTC 2025 賽季 (INTO THE DEEP) 影像辨識與策略分析自動計分系統
...
### FTC 比賽分析器 (新功能 🆕)
- **文物偵測**: 使用 HSV 顏色空間辨識紫色與綠色文物
- **機器人追蹤**: 追蹤全場 4 台機器人 (紅/藍聯盟各 2 台)
- **區域偵測**: 識別場地關鍵區域 (裝載區、發射區、坡道區等)
- **即時計分**: 實作完整 FTC 2025 計分規則
- **週期分析**: 追蹤機器人收球-移動-投放的效率
- **策略建議**: 根據週期時間評估效能等級與分數潛力
